...
class TrompDiagramGenerator {
  ...
  drawTermExact(term) {
    if (term.type === Term.TYPE.LAM) {
      ...
      // Account for the horizontal gap and shortened binder to ensure variables connect properly
      const lambdaGapAdjustment = unitSize * 1.0; // Same as gapSize in lambda rendering
      const verticalSpacingAdjustment = unitSize * 1.8; // Same as verticalSpacing in lambda rendering
      
      // Simple calculation directly from the Haskell example
      // Each variable's height is determined solely by its De Bruijn index
      const indexDistance = (term.index + 1); // De Bruijn index
      ...
      const leftTermColor = term.func && term.func.type === Term.TYPE.LAM && term.func.binderId && 
                           this.componentColors.has(term.func.binderId) ? 
                           this.componentColors.get(term.func.binderId) : appColor;
                           
      const rightTermColor = term.arg && term.arg.type === Term.TYPE.LAM && term.arg.binderId && 
                           this.componentColors.has(term.arg.binderId) ? 
                           this.componentColors.get(term.arg.binderId) : appColor;
